pro shrinkspherecenter_new,minparticles=minparticles,shrinkfactor=shrinkfactor,doplot=doplot,verbose=verbose,dostop=dostop
...
;    uniquebhgrp = bhgrp[uniq(bhgrp,sort(bhgrp))]
	...
	gind = where(grp[0:h.ngas-1] eq uniquegrp[i],ngas)			;section fixed by MJT to include possibility of no gas, no stars
	dind = where(grp[h.ngas:h.ngas+h.ndark-1] eq uniquegrp[i],ndark)
	sind = where(grp[h.ngas+h.ndark:h.n-1] eq uniquegrp[i],nstar)
        if ngas gt 0 then gas = g[gind]
	if ndark gt 0 then dark = d[dind]
	if nstar gt 0 then star = s[sind]
	...
	if ngas eq 0 and nstar eq 0 and ndark ne 0 then begin
	    masses = dark.mass
	    x = dark.x
	    y = dark.y
	    z = dark.z
		...
		gind = where(gasradius lt maxrad,ngas)
		if ngas ne 0 then gas = gas[gind]
	    endif
            if ndark ne 0 then begin
		dind = where(darkradius lt maxrad,ndark)
		if ndark ne 0 then dark = dark[dind]
	    endif
            if nstar ne 0 then begin
		sind = where(starradius lt maxrad,nstar)
		if nstar ne 0 then star = star[sind]
	    endif
